...
def exclude_files(files, platforms, skip_queued):
...
def find_images_in_folder(folder_path, extensions, platforms, skip_queued):
    image_paths = []
    for ext in extensions:
        files = glob.glob(os.path.join(folder_path, f"*{ext}"))
        filtered_files = exclude_files(files, platforms, skip_queued)
